        ...
        <>
            <footer className="footer bg-base-200 text-base-content grid-cols-1 gap-8 p-10 md:grid-cols-2 lg:grid-cols-4">
                <nav>
                    ...
                    {appSettings.data?.settings?.footerSiteNavigationCollection?.items.map((link) => (
                ...
                <nav>
                    ...
                    {appSettings.data?.settings?.footerServicesNavigationCollection?.items.map((link) => (
                        <Link key={link?.href} href={link?.href ?? ''} className="link link-hover">
                            {link?.title}
                        </Link>
                    ))}
                ...
                <nav>
                    ...
                    {appSettings.data?.settings?.footerLegalNavigationCollection?.items.map((link) => (
                        <Link key={link?.href} href={link?.href ?? ''} className="link link-hover">
                            {link?.title}
                        </Link>
                    ))}
